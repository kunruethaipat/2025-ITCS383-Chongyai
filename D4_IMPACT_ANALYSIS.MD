...
**Feature 1 — Mobile Client App (Mandatory)**
- R1: Mobile App Core Features
- R2: Mobile App Placeholder Screens (Conference, Chat & Interview Schedule)

**Feature 2 — Applicant Search & Invitation System**
- R3: Applicant Search (filter by skills, education, experience, job preference)
- R4: Send Job Invitation (recruiter views profile → sends invitation)
- R5: Respond to Invitation (applicant accept/reject → auto-creates application)

**Feature 3 — Job Recommendation System**
- R6: Similar Jobs on Job Detail Page (Web)
- R7: Similar Jobs on Job Detail Page (Mobile)
### Design
...
R1[R1: Mobile App Core]
R2[R2: Mobile App Placeholders]
R3[R3: Applicant Search]
R4[R4: Send Invitation]
R5[R5: Respond to Invitation]
R6[R6: Similar Jobs - Web]
R7[R7: Similar Jobs - Mobile]
end
...
%% C → C
%% Middleware dependency: every module depends on C1
%% Prisma model ownership: A queries model owned by B → A depends on B
C2 --> C1
C3 --> C1
C4 --> C1
C4 --> C3
...
### R2: Mobile App Placeholder Screens
...
- CR-02 (Fix CORS Configuration): Only the CORS configuration in app.ts was changed. No business logic or database schema was touched, so there is no ripple effect.

### Which change requests are difficult to apply and why?

- CR-05 (Applicant Search & Invitation System): A new Prisma model, migration, and multiple new endpoints were needed across backend, web, and mobile. SLO14 depends directly on SLO1, SLO2, SLO3, and SLO4. Accepting an invitation also auto-creates an Application, making SLO14 → SLO4 → SLO3 a chain that must all be tested together.

### To make the maintenance easier, what would you expect from the previous developers?

- Accurate C4 diagrams — Keep the C4 diagram accurate and up to date with the actual implementation.
